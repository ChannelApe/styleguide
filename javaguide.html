...
<div id="content">
<h1><a style="color: #000; text-decoration:none " href="https://www.channelape.com/"><img src="https://www.channelape.com/wp-content/themes/ChannelApe/images/logo.png" style="max-width:50px; vertical-align:bottom" /> ChannelApe Java Style Guide</a></h1>
...
character</strong> (<strong>0x20</strong>) and the 
<strong>ASCII Horizontal Tab</strong> (<strong>0x09</strong>) are the only whitespace characters that appear
...
<ol>
  ...
  <li>Tab characters are used for indentation.</li>
  <li>Space characters are <strong>not</strong> used for indentation.</li>
</ol>
...
<h3 id="s4.4-column-limit">4.4 Column limit: 120</h3>

<p>Java code has a column limit of 120 characters. A "character" means any Unicode code point.
...
<p>Comments should be used sparingly as they tend to go stale. If a comment seems necessary then 
maybe that is an indication that the code needs to be refactored to be more readable. If you absolutely 
must comment, use the following as a guide:</p>
...
<p><em>Unit Test</em> classes should end in Test (i.e. unit tests for <code class="prettyprint lang-java">Foo.class</code> would be in 
<code class="prettyprint lang-java">FooTest.class</code>). 
<br /><br />
Integration Test classes should end in IT (i.e. integration tests 
for <code class="prettyprint lang-java">Foo.class</code> would be in <code class="prettyprint lang-java">FooIT.class</code>). 
<br /><br />
Driver Test (unit or integration tests that
should not be picked up by build process) classed should end in Driver (i.e. driver 
tests for <code class="prettyprint lang-java">Foo.class</code> would be in <code class="prettyprint lang-java">FooDriver.class</code>)</p>
...
<p>Unit test names should reflect a '<a href="https://martinfowler.com/bliki/GivenWhenThen.html" target="_blank">givenWhenThen</a>' format in lowercase. 
  <ul>
      <li>The <strong>given</strong> part describes the state of the world before you begin
        the behavior you're specifying in this scenario. You can think of 
        it as the pre-conditions to the test.</li>
      <li>The <strong>when</strong> section is that behavior that you're specifying.</li>
      <li>Finally the <strong>then</strong> section describes the changes you expect due to the
        specified behavior.</li>
  </ul>
  
  Say you are testing the following method:
</p>
<p><code class="prettyprint lang-java">public boolean sendMessage(String message)</code></p>
<p>A test name might be: <code class="prettyprint lang-java">givenMessageWhenSendingMessageThenReturnTrue</code></p>
...
<p class="exception"><strong>Exception:</strong> In tests, a caught exception may not be ignored.
If you expect an exception to happen, it can be specified in the annotation of the test.</p>

<p><code class="prettyprint lang-java">@Test(expected = BusinessServiceException.class))</code></p>
...
<h3 id="s6.5-final-fields">6.5 Declaring fields and variables final where possible</h3>

<p>Variables and fields should be declared final where possible. Unless there is a reason
  for a varaible/field not to be final, then the variable/field should be final. </p>

  <pre class="prettyprint lang-java">
     private final String fieldName;

     public int method(final String p1) { 
       final String localVariable = "example";
      }
  </pre>

<a name="dont-return-variables-immediately"></a>
<h3 id="s6.6-dont-return-variables-immediately">6.6 Don't immediately return a variable after declaration</h3>

<p>Do not declare variables that are going to be immediately returned. </p>

  <pre class="prettyprint lang-java">
      //this
      public int add(final int num1, final int num2) { 
        int someNumber = num1 + num2;
        return someNumber
      }
...
<h4 id="s6.6.1-cycolmatic-complexity">6.7.1 Don't immediately return a variable after declaration</h4>
<p>Do not declare variables that are going to be immediately returned. </p>
...
<h4 id="s6.6.2-cycolmatic-complexity">6.7.2 Cyclomatic Complexity</h4>
<p>Ideally, keep cyclomatic complexity of a method to 10 or below. This 
  helps keeps things clean and concise.
</p>
<h4 id="s6.6.3-method-length">6.7.3 Length of Methods</h4>
<p>Methods should not be more than 100 lines.
</p>

<a name="javadoc"></a>
<h2 id="s7-javadoc">7 Javadoc</h2>

<p>Javadocs should only be used sparingly on public facing API methods. 
  As in comments in general, methods should be self explanatory. If not, then
  a refactoring may be needed. This section defines styles if you absolutely need to use them.</p>
